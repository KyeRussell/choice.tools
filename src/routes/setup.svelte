...
<div class="grid h-full grid-rows-[auto,1fr,auto] gap-2 md:gap-4">
	<p class="text-center text-2xl md:text-4xl">Add your choices to the list.</p>
	<div class="flex flex-col rounded border-4 border-neutral-700">
		...
		>
			{#each choices as choice}
				<li
					class="flex h-20 w-full items-center justify-between bg-neutral-100 p-4 align-middle md:h-32 md:p-8"
				>
					<span class="text-xl md:text-3xl">{choice}</span>
					<button
						type="submit"
						class="flex h-14 w-14 flex-shrink-0 items-center justify-center border-4 border-neutral-700 bg-amber-200 align-middle text-xl disabled:bg-gray-200 md:h-20 md:w-20 md:text-3xl"
